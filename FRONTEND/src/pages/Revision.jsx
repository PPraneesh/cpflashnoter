  ...
  if (!currentQuestion) {
    return (
      <div className="bg-neutral-900 min-h-[80vh] flex flex-col items-center justify-center p-4 text-neutral-400 whitespace-break-spaces">
        No questions left for today, check out <Link className="block text-blue-500 hover:underline" to="/analytics#recent-questions">{"recent revisions"}</Link>
      ...
      <div className="max-w-7xl mx-auto p-4 space-y-8">
        ...
        <div className="bg-neutral-800 border border-neutral-700/30 rounded-xl p-6 space-y-4">
          ...
          <div className="bg-neutral-700/50 border border-neutral-700/30 rounded-xl p-4">
            {showingSolution ? (
              <div className="space-y-2">
                <h3 className="text-sm text-neutral-400 font-medium">Solution</h3>
          ...
          <div className="bg-blue-900/20 border border-blue-500/20 rounded-xl p-4 flex flex-col md:flex-row md:items-center justify-between space-y-3 md:space-y-0">
            ...
            >
              {`${getHintButtonText()} (${(currentQuestion.hints?.length || 0) - (currentHintIndex + 1)} left)`}
            </button>
          </div>
         ...
         { currentQuestionIndex == traverseIndex && <div className="space-y-2">
            ...
            <div className="flex space-x-4">
              {[1, 2].map((value) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg border border-red-500/30 text-red-400 
                    hover:bg-red-500/10 ${confidence === value ? "bg-red-500/30 hover:bg-red-500/20" : ""}`}
              ...
              <button
                className={`px-4 py-2 rounded-lg border border-yellow-500/30 text-yellow-400 
                  hover:bg-yellow-500/10 ${confidence === 3 ? "bg-yellow-500/30 hover:bg-yellow-500/20" : ""}`}
              ...
              {[4, 5].map((value) => (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg border border-green-500/30 text-green-400 
                    hover:bg-green-500/10 ${confidence === value ? "bg-green-500/30 hover:bg-green-500/20" : ""}`}
                  onClick={() => setConfidence(value)}
                >
                  {value}
                </button>
        ...
        {/* Session Stats */}
        { currentQuestionIndex == traverseIndex && <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {/* Switches */}
          ...
          <div className="bg-neutral-800 border border-neutral-700/30 rounded-xl p-4">
            ...
            <div className="text-2xl font-bold">
              {analytics.shownHints.length}/{currentQuestion.hints?.length || 0}
            </div>
          </div>
          {/* Time */}
          <div className="bg-neutral-800 border border-neutral-700/30 rounded-xl p-4">
            ...
            <button
              onClick={() => {
                if (currentQuestionIndex > 0) {
                  ...
                  toast("There are no previous questions", { icon: <FaInfoCircle /> });
                } 
              }}
              className={`px-4 py-2 text-neutral-400 flex items-center border border-neutral-700/30  transition-all rounded-lg cursor-pointer  ${currentQuestionIndex > 0 ? "hover:border-neutral-600/50 bg-neutral-800 hover:bg-neutral-700/50" : "bg-neutral-800/50 "}`} 
            >
            ...
            <button
              onClick={() => {
                if (currentQuestionIndex < traverseIndex) {
                  ...
                  toast("complete the revision of the current question", { icon: <FaInfoCircle /> });
                } 
              }}
              className={`px-4 py-2 text-neutral-400 flex items-center border border-neutral-700/30  transition-all rounded-lg cursor-pointer ${currentQuestionIndex < traverseIndex ? "hover:border-neutral-600/50 bg-neutral-800 hover:bg-neutral-700/50" : "bg-neutral-800/50 "}`} 
