...
const Analytics = ({ short = false }) => {
  ...
  return (
    <div className={`bg-neutral-900 rounded-lg space-y-6${short ? " py-6" : " p-6"}`}>
        <div className="flex justify-between items-center">
          ...
          {short && <Link to="/analytics" className="px-6 py-2 bg-purple-700 text-white rounded-lg hover:bg-purple-600 transition-colors flex items-center">
            View Full Analytics
          </Link>}
        </div>
    ...
    <div className="bg-neutral-800 border border-neutral-700 rounded-xl p-6">
      <div className="flex flex-col items-center">
        <span className="text-5xl font-bold text-green-500">
          <CountUp end={data.overallStats.averageConfidence} decimals={1} />
        </span>
        ...
        <>
          <div className="bg-neutral-800 rounded-lg p-6">
            ...
            <div className="space-y-4">
              {data.categoryDistributions && data.categoryDistributions.length > 0 ? (() => {
                const total = data.categoryDistributions.reduce((acc, obj) => acc + obj.count, 0);
                return data.categoryDistributions.map((cat) => {
                  const percent = total ? Math.round((cat.count / total) * 100) : 0;
                  return (
                    <div key={cat.categoryName}>
                      <div className="flex justify-between items-center mb-2">
                        <span className="text-sm text-neutral-400">{cat.categoryName}</span>
                ...
                <div className="text-neutral-400">
                  No data available, Start <Link to="/generation" className="text-blue-500 hover:underline">generating</Link> notes and <Link to="/rev" className="text-blue-500 hover:underline">revising</Link> them
                </div>
              )}
            </div>
          </div>
              ...
              {activityData.map((day) => (
                <div
                  ...
                  className={`h-14 w-full rounded flex items-center justify-center text-white ${
                    day.count > 0 ? 'text-xs border border-green-700' : 'border border-neutral-700 hover:border-neutral-600 transition-all'
                  }`}
                  style={{
                    backgroundColor: day.count === 0 ? 'rgb(64,64 ,64,0.3)' : `rgba(34,197,94 , ${day.count / 12})`,
                  }}
            ...
            <div className="bg-neutral-800 border border-neutral-700 rounded-xl p-6">
              <div className="flex flex-col items-center">
                <span className="text-5xl font-bold text-blue-400 mb-2">
                  <CountUp end={data.overallStats.totalTimeSpent/60} decimals={2} />
                </span>
                <span className="text-lg text-white mb-1">Total Time Spent in min</span>
          ...
          <div className="bg-neutral-800 border border-neutral-700 p-6 rounded-lg" id="recent-questions">
            <h3 className="text-white text-xl font-semibold mb-4 flex items-center gap-2">
              Recent Questions
            </h3>
            <div className="space-y-2">
              {data.recentQuestions && data.recentQuestions.length > 0 ? data.recentQuestions.map((question) => (
                <div key={question.questionId} className='bg-neutral-700/30 border border-neutral-600/50 p-3 rounded transition-colors'>
                  <div className="flex justify-between items-center">
                    <Link
                      to={`/home/questions/${question.questionId}`}
                      className="block"
                    >
                      <span className="text-white hover:underline hover:underline-offset-2">{question.name} <HiOutlineExternalLink className='inline'/></span>
                    </Link>
